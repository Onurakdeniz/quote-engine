...
use crate::engine::quoting::{
    PriceQuote as RustPriceQuote, SinglePathQuote as RustSinglePathQuote,
}; // Alias Rust structs
...
impl From<RustPriceQuote> for PyPriceQuote {
    fn from(rs_quote: RustPriceQuote) -> Self {
        PyPriceQuote {
            ...
            fee_bps: rs_quote.fee_bps.map(|d| d.to_string()),
            gas_estimate: rs_quote.gas_estimate,
            path_details: rs_quote
                .path_details
                .into_iter()
                .map(PySinglePathQuote::from)
                .collect(),
            gross_amount_out: rs_quote.gross_amount_out,
            spread_bps: rs_quote.spread_bps.map(|d| d.to_string()),
...
impl PyPriceQuoter {
    ...
    fn new(
        ...
        let mut price_engine =
            PriceEngine::from_config(tracker, graph.clone(), cache.clone(), &config);
        if let Some(avg_gas_units) = avg_gas_units_per_swap {
            price_engine.avg_gas_units_per_swap = avg_gas_units;
        }
        if let Some(gas_price_gwei) = gas_price_gwei {
            if let Ok(mut gas_price) = price_engine.gas_price_wei.write() {
                *gas_price = u128::from(gas_price_gwei).saturating_mul(1_000_000_000);
            }
        }
            ...
            let mut updates = match updates_future.await {
                ...
                Err(e) => {
                    return Err(PyValueError::new_err(format!(
                        "Failed to establish update stream: {}",
                        e
                    )));
                }
            };

            if let Some(_event) = updates.recv().await {
                initial_engine_ref.update_graph_from_tracker_state();
                Ok(())
            } else {
                Err(PyValueError::new_err(
                    "Failed to get any data from initial stream.".to_string(),
                ))
            }
        })?;
    ...
    fn get_token_price_vs_numeraire(
        &self,
        ...
        future_into_py(py, async move {
            match engine.get_token_price(&token_bytes, None) {
                Some(price_decimal) => Ok(pyo3::Python::with_gil(|py| {
                    price_decimal.to_string().into_py(py)
                })),
    ...
    fn get_quote(
        ...
        let amount_in = amount_in_str
            .parse::<u128>()
            .map_err(|e| PyValueError::new_err(format!("Invalid amount_in_str: {}", e)))?;
        let k = k_paths.unwrap_or(1);

        future_into_py(py, async move {
            let rust_price_quote = engine
                .quote_multi(&token_in, &token_out, amount_in, k, None)
                .await;
